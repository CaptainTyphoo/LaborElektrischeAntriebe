...
\fancyhead[R]{v2.7\\www.fet.at} %Kopfzeile rechts
...
\begin{figure*}[!hp]
	...
	\path (0,0) coordinate (origin1);
	\path (1*8,0) coordinate (alpha);
	\path (0,0.6*8) coordinate (beta);
	\path (0.9780*6,0.2060*6) coordinate (is);
	\path (0.5*6,-0.866*6) coordinate (PsiR);
	\path (0.7738*6,-0.8083*6) coordinate (PsiS);
	\path (0.8227*6,0.475*6) coordinate (m1);
	\path (m1) ++(0.5*6,-0.866*6) coordinate (m2);
	
	\fill[gray!30,nearly transparent] (origin1) -- (m1) -- (m2) -- (PsiR) -- cycle;
	
	\draw [->] (origin1) -- (alpha);
	\draw [->] (origin1) -- (beta);
	\draw [->] (origin1) -- node[above]{$\i_S$} (is);
	\draw [->] (origin1) -- node[below left]{$\PPsi_R$} (PsiR);
	\draw [->] (origin1) -- node[above right]{$\PPsi_S$} (PsiS);
	\draw [->] (PsiR) -- node[below]{$\PPsi_\sigma$} (PsiS);
	\draw [-] (origin1) -- (m1);
	\draw [-] (m1) -- (m2);
	\draw [-] (PsiR) -- node[above=1cm] {$m$} (m2);
...
\begin{figure*}[!htbp]
	...
	\addplot[smooth,tension=0.7,blue] plot coordinates {
		(0 , 500)
		(25 , 450)
		(60 , 375)
		(80 , 320)
	...
	\addlegendentry{$U_A$}
	\addplot[domain=0:115, red] {1.591*x};
	\addlegendentry{a) $U_R$}
	\node [red] at (axis cs: 50,125) {a) stabil} ;
	\node [red] at (axis cs: 60,50) {a) instabil} ;
	\addplot[domain=20:115, green] {10000/x};
	\addlegendentry{c) $U_A$}
	\draw[color=black] (axis cs:22.1,456) circle (2pt)  node[above right] {stabil};
	\draw[color=black] (axis cs:97.7,102.8) circle (2pt)  node[below=0.3cm] {instabil};
	\end{axis}
	\end{tikzpicture}
\end{figure*}
...
&=\frac{\SI{400}{\volt}-\SI{55}{\volt}-\SI{375}{\volt}\cdot\arctan\mybr{\SI{0.004}{\radian\per\ampere}\cdot\SI{900}{\ampere}}\frac{\SI{1000}{\rounds\per\minute}}{\SI{1500}{\rounds\per\minute}}}{\SI{900}{\ampere}}\\
&=\SI{0.02226}{\ohm}\\
\eta&=\frac{P_{el,N}-R_A I_{A,N}^2}{P_{el,N}}\\
&=\frac{\SI{360}{\kilo\watt}-\SI{0.02226}{\ohm}\cdot\mybr{\SI{900}{\ampere}}^2}{\SI{360}{\watt}}\\
&=\num{0.9499}
\end{align}

\subsection{}
\begin{equation}
M=\frac{U_i\mybr{I_A} I_A}{2\pi}=\frac{\SI{55}{\volt}-\SI{375}{\volt}\cdot\arctan\mybr{\SI{0.004}{\radian\per\ampere}\cdot I_A}\frac{I_A \cdot\SI{60}{\second\per\minute}}{\SI{1500}{\rounds\per\minute}}}{2\pi}
\end{equation}
\begin{figure*}[!htbp]
	\centering
	\begin{tikzpicture}
	...
	xmin=0,
	ymin=0]
	\addplot[domain=0:1050, blue] {((55+375*atan(0.004*x)*pi/180)*x*60/1500)/(2*pi)};
	\draw[color=black] (axis cs:996,3500) circle (2pt);
	\end{axis}
	\end{tikzpicture}
\end{figure*}
...
&=\SI{0.02226}{\ohm}\cdot\SI{996}{\ampere}+\mybr{\SI{55}{\volt}+\SI{375}{\volt}\cdot\arctan\mybr{\SI{0.004}{\radian\per\ampere}\cdot\SI{996}{\ampere}}}\frac{\SI{600}{\rounds\per\minute}}{\SI{900}{\rounds\per\minute}}\\
&=\SI{390.1}{\volt}
\end{align}
...
&=\SI{0.02226}{\ohm}+\frac{\mybr{\SI{55}{\volt}+\SI{375}{\volt}\cdot\arctan\mybr{\SI{0.004}{\radian\per\ampere}\cdot\SI{1000}{\ampere}}}\frac{\SI{500}{\rounds\per\minute}}{\SI{1500}{\rounds\per\minute}}}{\SI{1000}{\ampere}}\\
&=\SI{0.2063}{\ohm}\\
n&=\frac{\mybr{R_B-R_A}I_A}{\frac{U_i\mybr{I_A}}{n_L}}\\
&=\frac{\mybr{\SI{0.2063}{\ohm}-\SI{0.02226}{\ohm}}\SI{500}{\ampere}}{\frac{\mybr{\SI{55}{\volt}+\SI{375}{\volt}\cdot\arctan\mybr{\SI{0.004}{\radian\per\ampere}\cdot\SI{500}{\ampere}}}}{\SI{1500}{\rounds\per\minute}}}\\
