...
\fancyhead[R]{v2.3\\www.fet.at} %Kopfzeile rechts
...
\begin{figure*}[!hp]
	\centering{
		...
		\draw [decorate,decoration={brace,amplitude=5pt},xshift=0pt,yshift=-3pt]
		(axis cs:60,400) -- (axis cs:27.5,400) node [black,midway,yshift=-13pt] 
		{$I_{sN}''$};
		...
		\draw[dashed,->] (axis cs:55.92,0) -- (axis cs:55.92,543);
		\draw[dashed,->] (axis cs:55.92,543) -- (axis cs:0,543) node[above right] {8) $U_{AL}$};
...
\section{}
\begin{align}
I_{sN}''&=I_{fIV}-I_{fK}=\SI{60}{\ampere}-\SI{27.5}{\ampere}=\SI{32.5}{\ampere}\\
\gamma&=\frac{I_{sN}''}{I_{sN}}=\frac{\SI{32.5}{\ampere}}{\SI{115.5}{\ampere}}=\num{0.2814}
\end{align}
...
\I_{sN,uv}''&=\SI{32.5}{\ampere}\e^{\j\SI{241.6}{\degree}}\quad\text{gleicher Winkel wie $\i_{s,uv}$}\\
\I_{f,uv}&=\I_{m,uv}-\I_{sN,uv}''=\SI{30.0}{\ampere}\e^{\j\SI{8.51}{\degree}}-\SI{32.5}{\ampere}\e^{\j\SI{241.6}{\degree}}=\SI{55.92}{\ampere}\e^{\j\SI{36.20}{\degree}}\\
&\text{$\mybr{0-\u_p-\u_i}$ und $\mybr{0-\I_f-\I_m}$ bilden ähnliche Dreiecke, daher gilt}\nonumber\\
\frac{I_m}{I_f}&=\frac{u_i}{u_p}\\
u_p&=\frac{u_i I_f}{I_m}=\frac{\num{1.100}\cdot\SI{55.92}{\ampere}}{\SI{30.0}{\ampere}}=\num{2.050}\\
\arg\mybr{\u_{p,uv}}&=\arg\mybr{\I_{f,uv}}+\SI{90}{\degree}=\SI{36.20}{\degree}+\SI{90}{\degree}=\SI{126.2}{\degree}\\
\u_{p,uv}&=\num{2.050}\e^{\j\SI{126.2}{\degree}}\\
\vartheta&=\arg\mybr{\u_{s,uv}}-\arg\mybr{\u_{p,uv}}=\SI{90}{\degree}-\SI{126.2}{\degree}=\SI{-36.20}{\degree}
\end{align}
\begin{figure*}[!ht]
	\centering
	\begin{tikzpicture}[>=triangle 45,thick,node distance=0.5cm]
	...
	\path (36.20:8) coordinate (d);
	\path (126.2:11) coordinate (q);
	\path (0,1*5) coordinate (us);
	\path (241.6:1*5) coordinate (is);
	\path (98.51:1.100*5) coordinate (ui);
	\path (8.51:30.0*0.1) coordinate (Im);
	\path (36.20:55.92*0.1) coordinate (If);
	\path (126.2:2.050*5) coordinate (up);
	\path (126.2:2.5) coordinate (arc1);
	...
	\draw [->] (arc1) arc (126.2:90:2.5) node[midway,below]{$\vartheta$};
...
\begin{align}
i_s&=0\\
I_f&=\mathrm{const.}=\SI{55.92}{\ampere}\\
u_s&=u_i=u_p\\
\text{aus Diagramm:}\nonumber\\
U_{AL}&=\SI{543}{\volt}
\end{align}
...
u_{S,d0}&=r_S i_d+l_d\frac{\d i_d}{\d\tau}\\
&\rotatebox[origin=c]{270}{\laplace}\nonumber\\
\frac{u_{S,d0}}{s}&=r_S+s l_d I_{S,d}-l_d i_{S,d0}\\
